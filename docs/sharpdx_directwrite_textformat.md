...
Source: https://developer.ninjatrader.com/docs/desktop/sharpdx_directwrite_textformat
...
- These properties cannot be changed after the `TextFormat` object is created. To change these properties, a new `TextFormat` object must be created with the desired properties.
- The **TextFormat** interface is used to draw text with a single format. To draw text with multiple formats, or to use a custom text renderer, use the [TextLayout](https://developer.ninjatrader.com/docs/desktop/sharpdx_directwrite_textlayout) interface. **TextLayout** enables the application to change the format for ranges of text within the string.
- This object may not be thread-safe, and it may carry the state of text format change.
- To draw simple text with a single format, Direct2D provides the [DrawText()](https://developer.ninjatrader.com/docs/desktop/sharpdx_direct2d1_rendertarget_drawtext) method, which draws a string using the format information provided by a **TextFormat** object.
