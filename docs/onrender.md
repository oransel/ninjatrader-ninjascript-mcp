...
Source: https://developer.ninjatrader.com/docs/desktop/onrender
...
- This method uses the 3rd party SharpDX library to render custom Direct2D Text and Shapes. For a walk through for using the SharpDX, please see the educational resource [Using SharpDX for Custom Chart Rendering](https://developer.ninjatrader.com/docs/desktop/using_sharpdx_for_custom_chart_rendering).
...
- When using the **Strategy Analyzer**, **OnRender()** does NOT call until you switch to the "Chart" display and renders from **State.Terminated**. As a result, this method should NOT be relied on for historical Strategy backtesting logic and should ONLY be used for rendering purposes.
...
- While **OnRender()** is an excellent means for customizing and enhancing indicators and strategies, its application can easily be abused, resulting in unforeseen performance issues which you may not catch until the right conditions (e.g., in the hands of your users during an FOMC event).
- Please limit any calculations or algorithms you may be tempted to run in **OnRender()** simply to rendering. You should always favor precomputed values and store them for rendering later as the preferred approach to working with the **OnRender()** method (e.g., reusing brushes, passing values from **OnBarUpdate()**, etc.). See also `OnRenderTargetChanged()` method for more information on reusing Brushes.
- If you are using this method as an opportunity to "hook" onto a user related event, such as when a user selects a 3rd party control, you should alternatively consider using the events of that control independent of official NinjaScript events. See also **TriggerCustomEvent()**.
